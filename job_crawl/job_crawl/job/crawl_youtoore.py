...
while True:
    ...
    if code !=200 or older_jobs_counter>=60:
        break
    print(code,'++',page_counter)
    result=json.loads(response.text)
    for each_job in range(len(result['vacancies'])):
        company=result['vacancies'][each_job]['company']['ident']
        title=result['vacancies'][each_job]['jobTitle']
        date=result['vacancies'][each_job]['fetchTime'].split('T')[0]
        link=result['vacancies'][each_job]['url']
    ...
    #print(company,place,title,date,link)
